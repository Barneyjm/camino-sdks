...
async def main():
    ...
    try:
        ...
        print(f"✅ Found {len(response.results)} Italian restaurants near Central Park")
    ...
    try:
        ...
        print(f"   Distance: {response.distance} ({response.actual_distance_km:.2f} km)")
    ...
    try:
        ...
        print(f"     Restaurants: {', '.join(response.relevant_places.restaurants)}")
        print(f"     Services: {', '.join(response.relevant_places.services)}")
        print(f"     Shops: {', '.join(response.relevant_places.shops)}")
        print(f"     Attractions: {', '.join(response.relevant_places.attractions)}")
    except APIError as e:
    ...
    try:
        waypoints = [
            ...
            Waypoint(lat=40.7505, lon=-73.9934, purpose="Empire State Building")
        ]
        ...
        for i, segment in enumerate(response.route_segments):
            print(f"   Segment {i+1}: {segment.from_.purpose} → {segment.to.purpose}")
            print(f"     Distance: {segment.distance_km} km, Time: {segment.estimated_time}")
    ...
    try:
        ...
        print("✅ Walking route:")
        print(f"   Distance: {response.summary.total_distance_meters:.1f} meters")
        print(f"   Duration: {response.summary.total_duration_seconds/60:.1f} minutes")
