...
class QueryRequest(BaseModel):
    ...
    q: str = Field(..., description="Natural language query, e.g., 'coffee near me'")
    lat: Optional[float] = Field(None, description="Latitude for the center of your search")
    lon: Optional[float] = Field(None, description="Longitude for the center of your search") 
    radius: Optional[int] = Field(None, description="Search radius in meters. Only used if lat/lon are provided.")
    rank: Optional[bool] = Field(True, description="Use AI to rank results by relevance (default: true)")
    limit: Optional[int] = Field(20, description="Maximum number of results to return (1-100, default: 20)", ge=1, le=100)
    offset: Optional[int] = Field(0, description="Number of results to skip for pagination (default: 0)", ge=0)
    answer: Optional[bool] = Field(False, description="Generate a human-readable answer summary (default: false)")
...
class QueryResult(BaseModel):
    ...
    cuisine: Optional[str] = Field(None, description="Cuisine type if applicable")
...
class RelationshipAnalysis(BaseModel):
    ...
    optimization_opportunities: List[str] = Field(..., description="List of optimization suggestions")
...
class ContextRequest(BaseModel):
    ...
    radius: Optional[str] = Field(None, description="Context radius with unit (e.g., '500m', '1km')")
    context: Optional[str] = Field(None, description="Context description for what to find")
    categories: Optional[List[str]] = Field(None, description="Specific categories to include")
...
class ContextResponse(BaseModel):
    ...
    relevant_places: RelevantPlaces = Field(..., description="Categorized places in the area")
...
class JourneyRequest(BaseModel):
    ...
    constraints: Optional[Dict[str, Any]] = Field(None, description="Journey constraints like transport mode and time budget")
...
class RouteSegment(BaseModel):
    ...
    instructions: Optional[str] = Field(None, description="Turn-by-turn instructions")
...
class JourneyResponse(BaseModel):
    ...
    total_distance_km: float = Field(..., description="Total journey distance in kilometers") 
    ...
    route_segments: List[RouteSegmentInfo] = Field(..., description="Journey route segments")
    analysis: RelationshipAnalysis = Field(..., description="Route analysis and optimization")
...
class RouteRequest(BaseModel):
    ...
    mode: Optional[str] = Field("foot", description="Transport mode (foot, car, bicycle)")
    include_geometry: Optional[bool] = Field(True, description="Include route geometry")
...
class RouteResponse(BaseModel):
    ...
    geometry: Optional[Dict[str, Any]] = Field(None, description="Route geometry data")
